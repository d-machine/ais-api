...
CREATE TABLE IF NOT EXISTS wms.item_brand (
    ...
    category_id INTEGER NOT NULL REFERENCES wms.item_category(id),
    descr VARCHAR(255),
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        INSERT INTO wms.item_brand_history (item_brand_id, brand_name, category_id, descr, is_active, operation, operation_at, operation_by)
        VALUES (NEW.id, NEW.brand_name, NEW.category_id, NEW.descr, NEW.is_active, 'INSERT', NEW.lua, NEW.lub);
    ELSIF TG_OP = 'UPDATE' THEN
        IF (OLD.is_active = true AND NEW.is_active = false) THEN
            INSERT INTO wms.item_brand_history (item_brand_id, brand_name, category_id, descr, is_active, operation, operation_at, operation_by)
            VALUES (NEW.id, NEW.brand_name, NEW.category_id, NEW.descr, NEW.is_active, 'DELETE', NEW.lua, NEW.lub);
        ELSE
            INSERT INTO wms.item_brand_history (item_brand_id, brand_name, category_id, descr, is_active, operation, operation_at, operation_by)
            VALUES (NEW.id, NEW.brand_name, NEW.category_id, NEW.descr, NEW.is_active, 'UPDATE', NEW.lua, NEW.lub);
        END IF;
    END IF;
    RETURN NEW;
END;
