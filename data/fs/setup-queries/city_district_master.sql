CREATE TABLE wms.city_district_master (
...
CREATE TABLE wms.city_district_master_history (
...
CREATE OR REPLACE FUNCTION wms.city_district_master_trigger()
RETURNS TRIGGER AS $$
BEGIN
  IF (TG_OP = 'INSERT') THEN
    INSERT INTO wms.city_district_master_history (city_district_id, city_id, district_id, state_id, description, operation, operation_at, operation_by)
...
DROP TRIGGER IF EXISTS city_district_trigger ON wms.city_district_master;
CREATE TRIGGER city_district_master_trigger
BEFORE INSERT OR UPDATE ON wms.city_district_master
FOR EACH ROW
EXECUTE FUNCTION wms.city_district_master_trigger();
