...
export default class AuthService extends BaseService {
    constructor() {
        super();
        this.JWT_SECRET = process.env.JWT_SECRET || "ais_dev_secret";
        this.TOKEN_EXPIRY = process.env.JWT_EXPIRATION_IN_HOURS || 12;
        this.REFRESH_TOKEN_EXPIRY = process.env.JWT_REFRESH_EXPIRATION_IN_DAYS || 30;
    ...
    async fetchAndCacheUserClaims(userId) {
      ...
      `, [userId]);
        if (userClaims) {
            _forEach(userClaims, async (row) => {
                ...
                await this._cache.saveSet(`user_access_type-${userId}`, _map(accessTypeIds, (accessTypeId) => `${row.resource_id}-${accessTypeId}`));
            });
        }
    }
    async saveRefreshToken(userId, token) {
        ...
        await this.executeSingleRowQuery("INSERT INTO administration.refresh_token (user_id, token, expires_at) VALUES ($1, $2, $3)", [userId, token, expiresAt]);
    }
    ...
    async refresh(refreshToken) {
        try {
            ...
            const refreshTokenRow = await this.executeSingleRowQuery("SELECT * FROM administration.refresh_token WHERE user_id = $1 AND token = $2", [decoded.userId, refreshToken]);
            if (!refreshTokenRow) {
