...
BEGIN
    IF TG_OP = 'INSERT' THEN
        INSERT INTO administration.role_history (role_id, name, description, team, department, operation, operation_at, operation_by)
        VALUES (NEW.id, NEW.name, NEW.description, NEW.team, NEW.department, 'INSERT', NEW.last_updated_at, NEW.last_updated_by);
    ELSIF TG_OP = 'UPDATE' THEN
        INSERT INTO administration.role_history (role_id, name, description, team, department, operation, operation_at, operation_by)
        VALUES (NEW.id, NEW.name, NEW.description, NEW.team, NEW.department, 'UPDATE', NEW.last_updated_at, NEW.last_updated_by);
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    INSERT INTO administration.role_history (
        role_id, name, description, team, department,
        operation, operation_at, operation_by
    )
    SELECT 
        id, name, description, team, department,
